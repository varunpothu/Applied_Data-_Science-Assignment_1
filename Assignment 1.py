...
def line_plot(data, stock1, stock2, stock3):
    
    
    ''' Defining Function to create a lineplot.
    Arguments: stock1, stock2, stock3 - used to call stocks
    ...
    plts.plot(data["DATE"], data[stock1], label = "WIPRO")
    plts.plot(data["DATE"], data[stock2], label = "TCS")
    plts.plot(data["DATE"], data[stock3], label = "HCLTECH")
...
def pie_chart(data1, Dept):
    
    
    ''' Defining Function to create a Pie chart.
    Arguments: Dept - used to call Department
    ...
    sum_by_dept = data1.groupby(Dept)['EmpID'].nunique().sort_values\
                                                          (ascending = False)
    ...
    plts.legend(title = Dept, bbox_to_anchor = (1, 0, 0.5, 1))
    plts.axis('equal')
...
def histogram(data1, wages):
    
    
    ''' Defining Function to create a histogram.
    Arguments: wages - used to call Salary
...
line_plot(data, 'WIPRO', 'TCS', 'HCLTECH')
pie_chart(data1, 'Department')
histogram(data1, 'Salary')
